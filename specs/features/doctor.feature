Feature: qmd doctor diagnostics
  ...
  # Checks: SQLite versions, embedding fingerprint freshness, mixed-fingerprint detection, safe legacy fingerprint adoption, content-hash sampling, device/GPU (with QMD_DOCTOR_DEVICE_PROBE), model cache, common env overrides (QMD_*, GGML_*), warnings for CPU etc.
